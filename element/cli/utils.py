...
def print_result(objs, log, as_json=False, as_pickle=False, depth=None, filter_tree=None):
    ...
    if as_json and (depth is not None or filter_tree is not None):
        log.error(
            "If you choose to print it as json, do not provide a depth or filter. Those are for printing it as a tree.")
        exit()
...
def get_result_as_tree(objs, depth=1, currentDepth=0, lastKey=""):
    ...
    if (isinstance(objs, dict)):
        stringToReturn += "\n"
        for key in objs:
            stringToReturn += currentDepth * "    " + key + ":   " + get_result_as_tree(objs[key], depth,
                                                                                        currentDepth + 1, key)
        return stringToReturn
...
def filter_objects(objs, keyPaths):
    ...
    for key in keyPaths:
        ...
        if key not in dictionaryOfInterest:
            raise ValueError("'" + key + "' is not a valid key for this level. Valid keys are: " + ','.join(
                dictionaryOfInterest.keys()))
...
def establish_connection(ctx):
    ...
    # If someone asked for a given connection or we need to default to using the connection at index 0 if it exists:
        ...
        if ctx.connectionindex is None and ctx.name is None:
            ...
            if int(ctx.connectionindex) > (len(connections) - 1) or int(ctx.connectionindex) < (-len(connections)):
                ctx.logger.error("Connection " + str(ctx.connectionindex) + " Please provide an index between " + str(
                    -len(connections)) + " and " + str(len(connections) - 1))
                exit(1)
            ...
            if (len(filteredCfg) > 1):
                ctx.logger.error(
                    "Your connections.csv file has become corrupted. There are two connections of the same name.")
                exit()
            if (len(filteredCfg) < 1):
        ...
        if cfg is not None:
            ...
            try:
                if int(cfg["port"]) != 443:
                    ...
                    address = cfg["mvip"]
                ctx.element = ElementFactory.create(address, cfg["username"], decrypt(cfg["password"]),
                                                    cfg["version"], verify_ssl=cfg["verifyssl"])
                if int(cfg["timeout"]) != 30:
                    ctx.element.timeout(cfg["timeout"])
            except Exception as e:
                ctx.logger.error(e.__str__())
                ctx.logger.error(
                    "The connection is corrupt. Run 'sfcli connection prune' to try and remove all broken connections or use 'sfcli connection remove -n name'")
    ...
    if ctx.json and ctx.element:
        def new_send_request(*args, **kwargs):
            return ctx.element.__class__.__bases__[0].send_request(ctx.element, return_response_raw=True, *args,
                                                                   **kwargs)
...
def get_connections(ctx):
    ...
    if os.path.exists(connectionsCsvLocation):
        try:
            ...
            ctx.logger.error("Problem reading " + connectionsCsvLocation + " because: " + str(
