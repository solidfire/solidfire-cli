...
@pass_context
def cli(ctx):
    """list removeinitiatorsfrom addvolumesto modifylunassignments modify removevolumesfrom delete create getlunassignments addinitiatorsto getefficiency """
...
def list(ctx,
    ...
    try:
        _ListVolumeAccessGroupsResult = ctx.element.list_volume_access_groups(startvolumeaccessgroupid=startvolumeaccessgroupid, limit=limit)
...
def modify(ctx,
    ...
    try:
        _ModifyVolumeAccessGroupResult = ctx.element.modify_volume_access_group(volume_access_group_id=volumeaccessgroupid, virtualnetworkid=virtualnetworkid, virtualnetworktags=virtualnetworktags, name=name, initiators=initiators, volumes=volumes, attributes=attributes)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def create(ctx,
    ...
    try:
        _CreateVolumeAccessGroupResult = ctx.element.create_volume_access_group(name=name, initiators=initiators, volumes=volumes, virtualnetworkid=virtualnetworkid, virtualnetworktags=virtualnetworktags, attributes=attributes)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
