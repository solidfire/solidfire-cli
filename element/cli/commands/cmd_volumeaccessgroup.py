...
@pass_context
def cli(ctx):
    """modifylunassignments list getefficiency create delete modify getlunassignments """
...
def modify(ctx,
    ...
    ctx.logger.info(""": """"""volumeaccessgroupid = """ + str(volumeaccessgroupid)+";" + """name = """+str(name)+";" + """initiators = """+str(initiators)+";" + """volumes = """+str(volumes)+";" + """deleteorphaninitiators = """+str(deleteorphaninitiators)+";" + """attributes = """+str(kwargsDict)+""";"""+"")
    try:
        _ModifyVolumeAccessGroupResult = ctx.element.modify_volume_access_group(volume_access_group_id=volumeaccessgroupid, name=name, initiators=initiators, volumes=volumes, delete_orphan_initiators=deleteorphaninitiators, attributes=kwargsDict)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
