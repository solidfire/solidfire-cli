...
@pass_context
def cli(ctx):
    """getclusteridentity resumerelationship modifyendpoint deleterelationships listvservers listrelationships getontapversioninfo listpolicies listaggregates breakrelationship listendpoints listschedules resyncrelationship quiescerelationship modifyendpointunmanaged createendpoint deleteendpoints initializerelationship modifyrelationship createvolume listvolumes listluns createrelationship abortrelationship createendpointunmanaged listnodes updaterelationship breakvolume listnetworkinterfaces """
...
@cli.command('deleterelationships', short_help="""The SolidFire Element OS web UI uses the DeleteSnapMirrorRelationships method to remove one or more SnapMirror relationships between a source and destination endpoint. """, cls=SolidFireCommand)
@click.option('--snapmirrorendpointid',
              type=int,
              required=True,
              prompt=True,
              help="""The endpoint ID of the remote ONTAP storage system communicating with the SolidFire cluster. """)
@click.option('--destinationvolumes',
              cls=SolidFireOption,
              is_flag=True,
              multiple=True,
              subparameters=["type", "volumeid", "vserver", "name", ],
              required=True,
              help="""The destination volume or volumes in the SnapMirror relationship.  Has the following subparameters: --type --volumeid --vserver --name """)
@click.option('--type',
              required=True,
              prompt=True,
              multiple=True,
              ...
              help="""[subparameter] The type of volume. Possible values: solidfire: The volume resides on a SolidFire cluster. ontap:     The volume resides on a remote ONTAP cluster. """,
              cls=SolidFireOption)
@click.option('--volumeid',
              required=False,
              multiple=True,
              ...
              default=None,
              is_sub_parameter=True,
              help="""[subparameter] The ID of the volume. Only valid if "type" is solidfire. """,
              cls=SolidFireOption)
@click.option('--vserver',
              required=False,
              multiple=True,
              ...
              default=None,
              is_sub_parameter=True,
              help="""[subparameter] The name of the Vserver that owns this volume. Only valid if "type" is ONTAP. """,
              cls=SolidFireOption)
@click.option('--name',
              required=True,
              prompt=True,
              ...
              default=None,
              is_sub_parameter=True,
              help="""[subparameter] The name of the volume. """,
              cls=SolidFireOption)
@pass_context
def deleterelationships(ctx,
    ...
    destinationvolumesArray = None
    if len(destinationvolumes) == 1 and type[0] is None and volumeid[0] is None and vserver[0] is None and name[0] is None:
        destinationvolumesArray = []
    elif(destinationvolumes is not None and destinationvolumes != ()):
        destinationvolumesArray = []
        try:
            for i, _destinationvolumes in enumerate(destinationvolumes):
                destinationvolumesArray.append(SnapMirrorVolumeInfo(type=type[i], volume_id=volumeid[i], vserver=vserver[i], name=name[i], ))
    ...
    ctx.logger.info(""": """"""snapmirrorendpointid = """ + str(snapmirrorendpointid)+";"+"""destinationvolumes = """ + str(destinationvolumesArray)+""";"""+"")
    try:
        _DeleteSnapMirrorRelationshipsResult = ctx.element.delete_snap_mirror_relationships(snap_mirror_endpoint_id=snapmirrorendpointid, destination_volumes=destinationvolumesArray)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def listrelationships(ctx,
    ...
    if(typedestinationvolume is not None or
       ...
       False):
        if not (typedestinationvolume and namedestinationvolume and  True):
            ctx.logger.error("""If you choose to provide , you must include all of the following parameters:
...
""")
    ...
    if(typesourcevolume is not None or
       ...
       False):
        if not (typesourcevolume and namesourcevolume and  True):
            ctx.logger.error("""If you choose to provide , you must include all of the following parameters:
typesourcevolume
namesourcevolume
""")
...
def createvolume(ctx,
    ...
    try:
        _SnapMirrorVolume = ctx.element.create_snap_mirror_volume(snap_mirror_endpoint_id=snapmirrorendpointid, vserver=vserver, name=name, aggregate=aggregate, size=size, type=type)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
        ...
        exit()
    if ctx.json:
        print(simplejson.dumps(simplejson.loads(_SnapMirrorVolume), indent=4))
        return
    else:
        cli_utils.print_result(_SnapMirrorVolume, ctx.logger, as_json=ctx.json, as_pickle=ctx.pickle, depth=ctx.depth, filter_tree=ctx.filter_tree)
