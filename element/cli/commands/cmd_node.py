...
@pass_context
def cli(ctx):
    """liststats setnetworkconfig adds removesslcertificate removes setsslcertificate getorigin getbootstrapconfig getpendingoperation listalls setconfig getnetworkconfig getstats getconfig listpendings listactives listpendingactives getsslcertificate """
...
@cli.command('adds', short_help="""AddNodes enables you to add one or more new nodes to a cluster. When a node that is not configured starts up for the first time, you are prompted to configure the node. After you configure the node, it is registered as a "pending node" with the cluster.  Note: It might take several seconds after adding a new node for it to start up and register its drives as available. """, cls=SolidFireCommand)
...
@cli.command('listalls', short_help="""ListAllNodes enables you to retrieve a list of active and pending nodes in the cluster. """, cls=SolidFireCommand)
@pass_context
def listalls(ctx):
...
@cli.command('listpendings', short_help="""ListPendingNodes returns a list of the currently pending nodes in the system. Pending nodes are nodes that are running and configured to join the cluster, but have not yet been added via the AddNodes API method. """, cls=SolidFireCommand)
@pass_context
def listpendings(ctx):
...
@cli.command('listactives', short_help="""ListActiveNodes returns the list of currently active nodes that are in the cluster. """, cls=SolidFireCommand)
@pass_context
def listactives(ctx):
...
@cli.command('listpendingactives', short_help="""ListPendingActiveNodes returns the list of nodes in the cluster that are currently in the PendingActive state, between the pending and active states. These are nodes that are currently being returned to the factory image. """, cls=SolidFireCommand)
@pass_context
def listpendingactives(ctx):
