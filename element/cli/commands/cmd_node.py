...
@pass_context
def cli(ctx):
    """listtats getorigin remove listpendingactive getpendingoperation setnetworkconfig removeslcertificate add setconfig getnetworkconfig listall getslcertificate getconfig setslcertificate gettats listactive listpending getbootstrapconfig """

@cli.command('listtats', short_help="""ListNodeStats enables you to view the high-level activity measurements for all nodes in a cluster. """, cls=SolidFireCommand)
@pass_context
def listtats(ctx):
...
@cli.command('removeslcertificate', short_help="""You can use the RemoveNodeSSLCertificate method to remove the user SSL certificate and private key for the management node. After the certificate and private key are removed, the management node is configured to use the default certificate and private key.. """, cls=SolidFireCommand)
@pass_context
def removeslcertificate(ctx):
...
@cli.command('getslcertificate', short_help="""You can use the GetNodeSSLCertificate method to retrieve the SSL certificate that is currently active on the cluster. You can use this method on both management and storage nodes. """, cls=SolidFireCommand)
@pass_context
def getslcertificate(ctx):
...
@cli.command('setslcertificate', short_help="""You can use the SetNodeSSLCertificate method to set a user SSL certificate and private key for the management node. """, cls=SolidFireCommand)
...
@cli.command('gettats', short_help="""GetNodeStats enables you to retrieve the high-level activity measurements for a single node. """, cls=SolidFireCommand)
@click.option('--nodeid',
              type=int,
              required=True,
              prompt=True,
