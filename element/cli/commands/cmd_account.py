...
@pass_context
def cli(ctx):
    """getbyid list add remove modify getbyname getefficiency """
...
def list(ctx,
    ...
    try:
        _ListAccountsResult = ctx.element.list_accounts(startaccountid=startaccountid, limit=limit)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def add(ctx,
    ...
    try:
        _AddAccountResult = ctx.element.add_account(username=username, initiatorsecret=initiatorsecret, targetsecret=targetsecret, attributes=attributes)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def modify(ctx,
    ...
    try:
        _ModifyAccountResult = ctx.element.modify_account(account_id=accountid, username=username, status=status, initiatorsecret=initiatorsecret, targetsecret=targetsecret, attributes=attributes)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
