...
@pass_context
def cli(ctx):
    """CreateSupportBundle DisableSnmp GetMasterNodeID GetCurrentAdmin GetSnmpTrapInfo AddAdmin GetVersionInfo GetInfo GetSnmpInfo SetNtpInfo GetState GetAPI GetSnmpState ClearFaults GetStats RemoveAdmin SnmpSendTestTraps ListEvents ListSyncJobs SetSnmpTrapInfo GetConfig Create GetFullThreshold GetSnmpACL SetSnmpACL SetConfig ModifyFullThreshold ListAdmins ModifyAdmin GetCapacity GetNtpInfo EnableSnmp SetSnmpInfo DisableEncryptionAtRest DeleteAllSupportBundles ListFaults GetLimits GetSystemStatus EnableEncryptionAtRest """
...
def CreateSupportBundle(ctx,
    ...
    try:
        _CreateSupportBundleResult = ctx.element.create_support_bundle(bundle_name=bundle_name, extra_args=extra_args, timeout_sec=timeout_sec)
...
def AddAdmin(ctx,
    ...
    try:
        _AddClusterAdminResult = ctx.element.add_cluster_admin(username=username, password=password, access=access, accept_eula=accept_eula, attributes=attributes)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def ClearFaults(ctx,
    ...
    try:
        _ClearClusterFaultsResult = ctx.element.clear_cluster_faults(fault_types=fault_types)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def RemoveAdmin(ctx,
    ...
    try:
        _RemoveClusterAdminResult = ctx.element.remove_cluster_admin(cluster_admin_id=cluster_admin_id)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def ListEvents(ctx,
    ...
    try:
        _ListEventsResult = ctx.element.list_events(max_events=max_events, start_event_id=start_event_id, end_event_id=end_event_id, event_queue_type=event_queue_type)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def SetSnmpTrapInfo(ctx,
    ...
    try:
        _SetSnmpTrapInfoResult = ctx.element.set_snmp_trap_info(trap_recipients=trap_recipients, cluster_fault_traps_enabled=cluster_fault_traps_enabled, cluster_fault_resolved_traps_enabled=cluster_fault_resolved_traps_enabled, cluster_event_traps_enabled=cluster_event_traps_enabled)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def Create(ctx,
    ...
    try:
        _CreateClusterResult = ctx.element.create_cluster(mvip=mvip, svip=svip, rep_count=rep_count, username=username, password=password, nodes=nodes, accept_eula=accept_eula, attributes=attributes)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def SetSnmpACL(ctx,
    ...
    try:
        _SetSnmpACLResult = ctx.element.set_snmp_acl(networks=networks, usm_users=usm_users)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def ModifyFullThreshold(ctx,
    ...
    try:
        _ModifyClusterFullThresholdResult = ctx.element.modify_cluster_full_threshold(stage2_aware_threshold=stage2_aware_threshold, stage3_block_threshold_percent=stage3_block_threshold_percent, max_metadata_over_provision_factor=max_metadata_over_provision_factor)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def ModifyAdmin(ctx,
    ...
    try:
        _ModifyClusterAdminResult = ctx.element.modify_cluster_admin(cluster_admin_id=cluster_admin_id, password=password, access=access, attributes=attributes)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def EnableSnmp(ctx,
    ...
    try:
        _EnableSnmpResult = ctx.element.enable_snmp(snmp_v3_enabled=snmp_v3_enabled)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def SetSnmpInfo(ctx,
    ...
    try:
        _SetSnmpInfoResult = ctx.element.set_snmp_info(networks=networks, enabled=enabled, snmp_v3_enabled=snmp_v3_enabled, usm_users=usm_users)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def ListFaults(ctx,
    ...
    try:
        _ListClusterFaultsResult = ctx.element.list_cluster_faults(exceptions=exceptions, best_practices=best_practices, update=update, fault_types=fault_types)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
