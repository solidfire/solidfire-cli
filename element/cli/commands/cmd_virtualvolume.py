...
@pass_context
def cli(ctx):
    """list getfeaturestatus listhosts listtasks enablefeature getcount listbindings """
...
def list(ctx,
    ...
    try:
        _ListVirtualVolumesResult = ctx.element.list_virtual_volumes(details=details, limit=limit, recursive=recursive, startvirtualvolumeid=startvirtualvolumeid, virtualvolumeids=virtualvolumeids)
...
def listhosts(ctx,
    ...
    try:
        _ListVirtualVolumeHostsResult = ctx.element.list_virtual_volume_hosts(virtualvolumehostids=virtualvolumehostids)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def listtasks(ctx,
    ...
    try:
        _ListVirtualVolumeTasksResult = ctx.element.list_virtual_volume_tasks(virtualvolumetaskids=virtualvolumetaskids)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def listbindings(ctx,
    ...
    try:
        _ListVirtualVolumeBindingsResult = ctx.element.list_virtual_volume_bindings(virtualvolumebindingids=virtualvolumebindingids)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
