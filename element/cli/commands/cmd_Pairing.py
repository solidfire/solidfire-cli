...
@pass_context
def cli(ctx):
    """CompleteVolume StartCluster StartVolume RemoveClusterPair RemoveVolumePair CompleteCluster ListActivePairedVolumes ListClusterPairs ModifyVolumePair """
...
def CompleteVolume(ctx,
    ...
    try:
        _CompleteVolumePairingResult = ctx.element.complete_volume_pairing(volume_pairing_key=volume_pairing_key, volume_id=volume_id)
...
def StartVolume(ctx,
    ...
    try:
        _StartVolumePairingResult = ctx.element.start_volume_pairing(volume_id=volume_id, mode=mode)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def RemoveClusterPair(ctx,
    ...
    try:
        _RemoveClusterPairResult = ctx.element.remove_cluster_pair(cluster_pair_id=cluster_pair_id)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def RemoveVolumePair(ctx,
    ...
    try:
        _RemoveVolumePairResult = ctx.element.remove_volume_pair(volume_id=volume_id)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def CompleteCluster(ctx,
    ...
    try:
        _CompleteClusterPairingResult = ctx.element.complete_cluster_pairing(cluster_pairing_key=cluster_pairing_key)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def ModifyVolumePair(ctx,
    ...
    try:
        _ModifyVolumePairResult = ctx.element.modify_volume_pair(volume_id=volume_id, paused_manual=paused_manual, mode=mode)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
