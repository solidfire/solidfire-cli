...
@pass_context
def cli(ctx):
    """listbulkjobs listdeleted create getdefaultqos cancelclone cancelgroupclone clone startbulkread getstats createqospolicy liststatsbyaccessgroup liststatsbyaccount listactive startbulkwrite removefromaccessgroup listqoshistograms deleteqospolicy liststatsby setdefaultqos list getqospolicy listforaccount modify delete addtoaccessgroup updatebulkstatus purgedeleted modifyqospolicy restoredeleted liststats getefficiency clonemultiple copy listqospolicies getcount """
...
@click.option('--curve',
              type=dict,
              required=False,
              help="""The curve is a set of key-value pairs. The keys are I/O sizes in bytes. The values represent the cost of performing an IOP at a specific I/O size. The curve is calculated relative to a 4096 byte operation set at 100 IOPS. """)
...
@click.option('--access',
              type=str,
              required=False,
              help="""The access mode for the volume. Only snapMirrorTarget is allowed. """)
@click.option('--enablesnapmirrorreplication',
              type=bool,
              required=False,
              help="""Specifies whether SnapMirror replication is enabled or not. """)
...
@click.option('--protectionscheme',
              type=ProtectionScheme,
              required=False,
              help="""Protection scheme that should be used for this volume. The default value is the defaultProtectionScheme stored in the ClusterInfo object. """)
@pass_context
def create(ctx,
...
""")
    ...
    if protectionscheme is not None:
        protectionscheme = protectionscheme.get_value()

    ctx.logger.info(""": """"""name = """ + str(name)+";"+"""accountid = """ + str(accountid)+";"+"""totalsize = """ + str(totalsize)+";" + """enable512e = """+str(enable512e)+";" + """qos = """+str(qos)+";" + """attributes = """+str(kwargsDict)+";" + """associatewithqospolicy = """+str(associatewithqospolicy)+";" + """access = """+str(access)+";" + """enablesnapmirrorreplication = """+str(enablesnapmirrorreplication)+";" + """qospolicyid = """+str(qospolicyid)+";" + """protectionscheme = """+str(protectionscheme)+""";"""+"")
    try:
        _CreateVolumeResult = ctx.element.create_volume(name=name, account_id=accountid, total_size=totalsize, enable512e=enable512e, qos=qos, attributes=kwargsDict, associate_with_qos_policy=associatewithqospolicy, access=access, enable_snap_mirror_replication=enablesnapmirrorreplication, qos_policy_id=qospolicyid, protection_scheme=protectionscheme)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
@click.option('--name',
              ...
              required=True,
              prompt=True,
              help="""The name of the new cloned volume. Must be 1 to 64 characters in length. """)
...
@click.option('--newsize',
              type=int,
              required=False,
              help="""New size of the volume, in bytes. Must be greater or less than the size of the volume being cloned. If unspecified, the volume size is not changed. Size is rounded to the nearest 1MB. """)
@click.option('--access',
              type=VolumeAccess,
              required=False,
              help="""Specifies the level of access allowed for the new volume. If unspecified, the level of access of the volume being cloned is used. If replicationTarget is is passed and the volume is not paired, the access gets set to locked. """)
...
@click.option('--enable512e',
              type=bool,
              required=False,
              help="""Specifies whether the new volume should use 512-byte sector emulation. If unspecified, the setting of the volume being cloned is used. """)
@click.option('--enablesnapmirrorreplication',
              type=bool,
              required=False,
              help="""Specifies whether SnapMirror replication is enabled or not. Defaults to false. """)
@pass_context
def clone(ctx,
           # Mandatory main parameter
           volumeid,
    ...
    ctx.logger.info(""": """"""volumeid = """ + str(volumeid)+";"+"""name = """ + str(name)+";" + """newaccountid = """+str(newaccountid)+";" + """newsize = """+str(newsize)+";" + """access = """+str(access)+";" + """snapshotid = """+str(snapshotid)+";" + """attributes = """+str(kwargsDict)+";" + """enable512e = """+str(enable512e)+";" + """enablesnapmirrorreplication = """+str(enablesnapmirrorreplication)+""";"""+"")
    try:
        _CloneVolumeResult = ctx.element.clone_volume(volume_id=volumeid, name=name, new_account_id=newaccountid, new_size=newsize, access=access, snapshot_id=snapshotid, attributes=kwargsDict, enable512e=enable512e, enable_snap_mirror_replication=enablesnapmirrorreplication)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
@cli.command('listqoshistograms', short_help="""ListVolumeQoSHistograms returns histograms detailing volume performance relative to QOS settings. It may take up to 5 seconds for newly created volumes to have accurate histogram data available. """, cls=SolidFireCommand)
@click.option('--volumeids',
              type=int,
              required=False,
              help="""List of volumes to return data for. If no volumes are specified then information for all volumes will be returned. """)
@pass_context
def listqoshistograms(ctx,
           # Optional main parameter
           volumeids = None):
    ...
    try:
        _ListVolumeQoSHistogramsResult = ctx.element.list_volume_qos_histograms(volume_ids=volumeids)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
        ...
        exit()
    if ctx.json:
        print(simplejson.dumps(simplejson.loads(_ListVolumeQoSHistogramsResult), indent=4))
        return
    else:
        cli_utils.print_result(_ListVolumeQoSHistogramsResult, ctx.logger, as_json=ctx.json, as_pickle=ctx.pickle, depth=ctx.depth, filter_tree=ctx.filter_tree)
...
@click.option('--includevirtualvolumes',
              ...
              required=False,
              help="""Specifies that virtual volumes are included in the response by default. To exclude virtual volumes, set to false. """)
@click.option('--protectionschemes',
              type=str,
              required=False,
              help="""Only volumes that are using one of the protection schemes in this set are returned. """)
@pass_context
def list(ctx,
           # Optional main parameter
           startvolumeid = None,
    ...
    protectionschemes = parser.parse_array(protectionschemes)
    

    

    ctx.logger.info(""": """"""startvolumeid = """+str(startvolumeid)+";" + """limit = """+str(limit)+";" + """volumestatus = """+str(volumestatus)+";" + """accounts = """+str(accounts)+";" + """ispaired = """+str(ispaired)+";" + """volumeids = """+str(volumeids)+";" + """volumename = """+str(volumename)+";" + """includevirtualvolumes = """+str(includevirtualvolumes)+";" + """protectionschemes = """+str(protectionschemes)+""";"""+"")
    try:
        _ListVolumesResult = ctx.element.list_volumes(start_volume_id=startvolumeid, limit=limit, volume_status=volumestatus, accounts=accounts, is_paired=ispaired, volume_ids=volumeids, volume_name=volumename, include_virtual_volumes=includevirtualvolumes, protection_schemes=protectionschemes)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
