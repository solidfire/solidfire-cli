...
@pass_context
def cli(ctx):
    """listbulkjobs listdeleted cancelclone modify listasyncresults getcount create list listactive restoredeleted getstats getdefaultqos listforaccount setdefaultqos liststatsbyaccessgroup startbulkwrite getefficiency getasyncresult liststatsby cancelgroupclone delete clonemultiple copy clone purgedeleted liststatsbyaccount updatebulkstatus startbulkread """
...
def modify(ctx,
    ...
    try:
        _ModifyVolumeResult = ctx.element.modify_volume(volume_id=volumeid, accountid=accountid, access=access, qos=qos, totalsize=totalsize, attributes=attributes)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def listasyncresults(ctx,
    ...
    try:
        _ListAsyncResultsResult = ctx.element.list_async_results(asyncresulttypes=asyncresulttypes)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def list(ctx,
    ...
    try:
        _ListVolumesResult = ctx.element.list_volumes(startvolumeid=startvolumeid, limit=limit, volumestatus=volumestatus, accounts=accounts, ispaired=ispaired, volumeids=volumeids)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def listactive(ctx,
    ...
    try:
        _ListActiveVolumesResult = ctx.element.list_active_volumes(startvolumeid=startvolumeid, limit=limit)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def listforaccount(ctx,
    ...
    try:
        _ListVolumesForAccountResult = ctx.element.list_volumes_for_account(account_id=accountid, startvolumeid=startvolumeid, limit=limit)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def setdefaultqos(ctx,
    ...
    try:
        _SetDefaultQoSResult = ctx.element.set_default_qos(miniops=miniops, maxiops=maxiops, burstiops=burstiops)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def liststatsbyaccessgroup(ctx,
    ...
    try:
        _ListVolumeStatsByVolumeAccessGroupResult = ctx.element.list_volume_stats_by_volume_access_group(volumeaccessgroups=volumeaccessgroups)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def startbulkwrite(ctx,
    ...
    try:
        _StartBulkVolumeWriteResult = ctx.element.start_bulk_volume_write(volume_id=volumeid, format=format, script=script, scriptparameters=scriptparameters, attributes=attributes)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def clonemultiple(ctx,
    ...
    try:
        _CloneMultipleVolumesResult = ctx.element.clone_multiple_volumes(volumes=volumes, access=access, groupsnapshotid=groupsnapshotid, newaccountid=newaccountid)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def copy(ctx,
    ...
    try:
        _CopyVolumeResult = ctx.element.copy_volume(volume_id=volumeid, dst_volume_id=dstvolumeid, snapshotid=snapshotid)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def clone(ctx,
    ...
    try:
        _CloneVolumeResult = ctx.element.clone_volume(volume_id=volumeid, name=name, newaccountid=newaccountid, newsize=newsize, access=access, snapshotid=snapshotid, attributes=attributes)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def updatebulkstatus(ctx,
    ...
    try:
        _UpdateBulkVolumeStatusResult = ctx.element.update_bulk_volume_status(key=key, status=status, percentcomplete=percentcomplete, message=message, attributes=attributes)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def startbulkread(ctx,
    ...
    try:
        _StartBulkVolumeReadResult = ctx.element.start_bulk_volume_read(volume_id=volumeid, format=format, snapshotid=snapshotid, script=script, scriptparameters=scriptparameters, attributes=attributes)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
