...
@pass_context
def cli(ctx):
    """getcurrentadmin listadmins setconfig getapi enableencryptionatrest create getntpinfo snmpsendtesttraps getstats getconfig listevents getlimits getsnmpacl getstate getcapacity getsnmptrapinfo enablesnmp getversioninfo createsupportbundle disableencryptionatrest getmasternodeid getfullthreshold listsyncjobs modifyfullthreshold deleteallsupportbundles modifyadmin setntpinfo disablesnmp addadmin removeadmin setsnmptrapinfo setsnmpinfo clearfaults listfaults getsnmpstate getsnmpinfo getinfo getsystemstatus setsnmpacl """
...
def create(ctx,
    ...
    try:
        _CreateClusterResult = ctx.element.create_cluster(mvip=mvip, svip=svip, rep_count=repcount, username=username, password=password, nodes=nodes, accepteula=accepteula, attributes=attributes)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def listevents(ctx,
    ...
    try:
        _ListEventsResult = ctx.element.list_events(maxevents=maxevents, starteventid=starteventid, endeventid=endeventid, eventqueuetype=eventqueuetype)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def createsupportbundle(ctx,
    ...
    try:
        _CreateSupportBundleResult = ctx.element.create_support_bundle(bundlename=bundlename, extraargs=extraargs, timeoutsec=timeoutsec)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def modifyfullthreshold(ctx,
    ...
    try:
        _ModifyClusterFullThresholdResult = ctx.element.modify_cluster_full_threshold(stage2awarethreshold=stage2awarethreshold, stage3blockthresholdpercent=stage3blockthresholdpercent, maxmetadataoverprovisionfactor=maxmetadataoverprovisionfactor)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def addadmin(ctx,
    ...
    try:
        _AddClusterAdminResult = ctx.element.add_cluster_admin(username=username, password=password, access=access, accepteula=accepteula, attributes=attributes)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def setsnmptrapinfo(ctx,
    ...
    traprecipients = parser.parse_array(traprecipients)
...
def setsnmpinfo(ctx,
    ...
    usmusers = parser.parse_array(usmusers)

    ctx.logger.info("""networks = """+str(networks)+""";"""+"""enabled = """+str(enabled)+""";"""+"""snmpv3enabled = """+str(snmpv3enabled)+""";"""+"""usmusers = """+str(usmusers)+""";"""+"")
    try:
        _SetSnmpInfoResult = ctx.element.set_snmp_info(networks=networks, enabled=enabled, snmpv3enabled=snmpv3enabled, usmusers=usmusers)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def clearfaults(ctx,
    ...
    try:
        _ClearClusterFaultsResult = ctx.element.clear_cluster_faults(faulttypes=faulttypes)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def listfaults(ctx,
    ...
    try:
        _ListClusterFaultsResult = ctx.element.list_cluster_faults(exceptions=exceptions, bestpractices=bestpractices, update=update, faulttypes=faulttypes)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
