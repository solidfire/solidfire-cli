...
@pass_context
def cli(ctx):
    """reset secureerase getdriveconfig list listtats getdrivehardwareinfo remove add listdrivehardware test gettats """
...
@cli.command('listtats', short_help="""ListDriveStats enables you to retrieve high-level activity measurements for multiple drives in the cluster. By default, this method returns statistics for all drives in the cluster, and these measurements are cumulative from the addition of the drive to the cluster. Some values this method returns are specific to block drives, and some are specific to metadata drives. """, cls=SolidFireCommand)
@click.option('--drives',
              type=str,
              required=False,
...
@cli.command('getdrivehardwareinfo', short_help="""GetDriveHardwareInfo returns all the hardware information for the given drive. This generally includes details about manufacturers, vendors, versions, and other associated hardware identification information. """, cls=SolidFireCommand)
...
@click.option('--forceduringbinsync',
              type=bool,
              required=False,
              help="""If you want to remove a drive during a Bin Sync, this must be set to true. """)
@pass_context
def remove(ctx,
           # Mandatory main parameter
           drives,
    ...
    ctx.logger.info(""": """"""drives = """ + str(drives)+";" + """forceduringupgrade = """+str(forceduringupgrade)+";" + """forceduringbinsync = """+str(forceduringbinsync)+""";"""+"")
    try:
        _AsyncHandleResult = ctx.element.remove_drives(drives=drives, force_during_upgrade=forceduringupgrade, force_during_bin_sync=forceduringbinsync)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
@cli.command('gettats', short_help="""GetDriveStats returns high-level activity measurements for a single drive. Values are cumulative from the addition of the drive to the cluster. Some values are specific to block drives. You might not obtain statistical data for both block and metadata drives when you run this method.  """, cls=SolidFireCommand)
@click.option('--driveid',
              type=int,
              required=True,
              prompt=True,
