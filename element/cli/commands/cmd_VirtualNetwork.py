...
def Add(ctx,
    ...
    try:
        _AddVirtualNetworkResult = ctx.element.add_virtual_network(virtual_network_tag=virtual_network_tag, name=name, address_blocks=address_blocks, netmask=netmask, svip=svip, gateway=gateway, namespace=namespace, attributes=attributes)
...
def Modify(ctx,
    ...
    try:
        _AddVirtualNetworkResult = ctx.element.modify_virtual_network(virtual_network_id=virtual_network_id, virtual_network_tag=virtual_network_tag, name=name, address_blocks=address_blocks, netmask=netmask, svip=svip, gateway=gateway, namespace=namespace, attributes=attributes)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
