...
@pass_context
def cli(ctx):
    """list connectensemble connectmvip connectsvip listutilities ping """
...
def ping(ctx,
    ...
    try:
        _TestPingResult = ctx.element.test_ping(attempts=attempts, hosts=hosts, totaltimeoutsec=totaltimeoutsec, packetsize=packetsize, pingtimeoutmsec=pingtimeoutmsec)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
