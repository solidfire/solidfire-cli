...
@pass_context
def cli(ctx):
    """getcompletestats getrawstats listdrivestats gethardwareinfo listvolumestats listvolumestatsbyvirtualvolume """
...
def listvolumestats(ctx,
    ...
    try:
        _ListVolumeStatsResult = ctx.element.list_volume_stats(volumeids=volumeids)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def listvolumestatsbyvirtualvolume(ctx,
    ...
    try:
        _ListVolumeStatsByVirtualVolumeResult = ctx.element.list_volume_stats_by_virtual_volume(virtualvolumeids=virtualvolumeids)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
