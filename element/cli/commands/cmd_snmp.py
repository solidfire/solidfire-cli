...
@pass_context
def cli(ctx):
    """getsnmpacl setsnmpinfo gettate getsnmpinfo setsnmpacl getsnmptrapinfo disablesnmp setsnmptrapinfo endtesttraps enablesnmp """
...
@cli.command('gettate', short_help="""You can use GetSnmpState to return the current state of the SNMP feature. """, cls=SolidFireCommand)
@pass_context
def gettate(ctx):
...
@cli.command('endtesttraps', short_help="""SnmpSendTestTraps enables you to test SNMP functionality for a cluster. This method instructs the cluster to send test SNMP traps to the currently configured SNMP manager. """, cls=SolidFireCommand)
@pass_context
def endtesttraps(ctx):
