...
@pass_context
def cli(ctx):
    """setacl gettrapinfo sendtesttraps settrapinfo getstate enable setinfo getacl getinfo disable """
...
def settrapinfo(ctx,
    ...
    ctx.logger.info(""": """"""clusterfaulttrapsenabled = """ + str(clusterfaulttrapsenabled)+";"+"""clusterfaultresolvedtrapsenabled = """ + str(clusterfaultresolvedtrapsenabled)+";"+"""clustereventtrapsenabled = """ + str(clustereventtrapsenabled)+";" + """traprecipients = """+str(traprecipientsArray)+""";"""+"")
    try:
        _SetSnmpTrapInfoResult = ctx.element.set_snmp_trap_info(cluster_fault_traps_enabled=clusterfaulttrapsenabled, cluster_fault_resolved_traps_enabled=clusterfaultresolvedtrapsenabled, cluster_event_traps_enabled=clustereventtrapsenabled, trap_recipients=traprecipientsArray)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
