...
@pass_context
def cli(ctx):
    """completevolume startvolume listactivepairedvolumes removevolumepair modifyvolumepair completecluster removeclusterpair listclusterpairs startcluster """
...
def modifyvolumepair(ctx,
    ...
    try:
        _ModifyVolumePairResult = ctx.element.modify_volume_pair(volume_id=volumeid, pausedmanual=pausedmanual, mode=mode)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
