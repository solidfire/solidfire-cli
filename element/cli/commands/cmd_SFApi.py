...
def Invoke(ctx,
    ...
    try:
        strr = ctx.element.invoke_sfapi(method=method, parameters=parameters)
    ...
    cli_utils.print_result(strr, ctx.logger, as_json=ctx.json, as_pickle=ctx.pickle, depth=ctx.depth, filter_tree=ctx.filter_tree)
