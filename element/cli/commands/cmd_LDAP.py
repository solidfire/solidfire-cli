...
@pass_context
def cli(ctx):
    """EnableLdapAuthentication AddLdapClusterAdmin GetLdapConfiguration TestLdapAuthentication DisableLdapAuthentication """
...
def EnableLdapAuthentication(ctx,
    ...
    try:
        _EnableLdapAuthenticationResult = ctx.element.enable_ldap_authentication(server_uris=server_uris, auth_type=auth_type, group_search_base_dn=group_search_base_dn, group_search_custom_filter=group_search_custom_filter, group_search_type=group_search_type, search_bind_dn=search_bind_dn, search_bind_password=search_bind_password, user_dntemplate=user_dntemplate, user_search_base_dn=user_search_base_dn, user_search_filter=user_search_filter)
...
def AddLdapClusterAdmin(ctx,
    ...
    try:
        _AddLdapClusterAdminResult = ctx.element.add_ldap_cluster_admin(username=username, access=access, accept_eula=accept_eula, attributes=attributes)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
...
def TestLdapAuthentication(ctx,
    ...
    try:
        _TestLdapAuthenticationResult = ctx.element.test_ldap_authentication(username=username, password=password, ldap_configuration=ldap_configuration)
    except common.ApiServerError as e:
        ctx.logger.error(e.message)
        exit()
    except BaseException as e:
